...
public class Menu_stok extends roundednew {
    ...
    String id = "001"; // default ID
    try {
        Connection conn = Koneksi.koneksi();
        Statement st = conn.createStatement();
        ResultSet rs = st.executeQuery("SELECT MAX(Id_BahanBaku) FROM bahan_baku");
        if (rs.next() && rs.getString(1) != null) {
            String lastId = rs.getString(1); // ambil ID terakhir, misal "005"
            int nextId = Integer.parseInt(lastId) + 1;
            id = String.format("%03d", nextId); // hasil: 006
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
    ...
    private void loadData(){
        try {
           ...
           ResultSet rs = pst.executeQuery();
            DefaultTableModel model = new DefaultTableModel(new String[]{"Id Bahan","Nama Bahan", "Jumlah Bahan", "Harga Beli"}, 0);
            jTable1.setModel(model);
            while (rs.next()) {
                Object[] row = {
                  rs.getString("Id_BahanBaku"),
                  rs.getString("Nama_BahanBaku"),
                  rs.getString("Jumlah_BahanBaku"),
    ...
    private void tambahStok(){
        try {
            ...
            String query = "INSERT INTO bahan_baku(Id_BahanBaku,Nama_BahanBaku,Jumlah_BahanBaku,Tanggal,Harga_beli) VALUES (?,?,?,CURRENT_DATE,?)";
            PreparedStatement pst = conn.prepareStatement(query);
            
            pst.setString(1, idBaru);
            pst.setString(2, txt_nama.getText());
            pst.setString(3, txt_jum.getText());
            pst.setString(4, txt_harga.getText());
            pst.executeUpdate();
            
            JOptionPane.showMessageDialog(null, " Bahan Baku berhasil ditambahkan");
        loadData();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Gagal Menambahkan Bahan Baku" + e.getMessage());
        }
    }
    ...
    private void editStok(int rowIndex){
        try {
            ...
            String query = "UPDATE bahan_baku SET Nama_BahanBaku = ?, Harga_beli = ? WHERE Id_BahanBaku= ? ";
            PreparedStatement pst = conn.prepareStatement(query);
    ...
    private void tambahJumlah(int rowIndex){
        try {
           ...
           double jumlahBaru = jumlahLama + jumlahTambah;
            String query = "UPDATE restok set Jumlah_BahanBaku=?  WHERE Id_BahanBaku=?";
            PreparedStatement pst = conn.prepareStatement(query);
            ...
            if (row>0) {
                JOptionPane.showMessageDialog(null, "Jumlah Bahan Baku Berhasil di Tambahkan", "Update", 0);
    ...
    private void initComponents() {
        ...
        judul.setText("Tambah Bahan Baku");
        ...
        jLabel3.setText("Jumlah Bahan Baku");
        ...
        jLabel8.setText("Harga Bahan Baku");
        roundednew1.add(jLabel8, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 180, -1, 31));
